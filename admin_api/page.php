...
elseif($_POST['func']=='active_list')
{
	$returnArray['success'] = true;
	$returnArray['msg'] = "Data Found.";
	$sql = "select * from page where is_active='1' order by title";
	$res = mysqli_query($link, $sql) or die(mysqli_error($link));	
	$i=0;
	while($row = mysqli_fetch_array($res))
	{
...
elseif($_POST['func'] == "status_change")
{
	...
	$returnArray['msg'] = "Page status change is not successful.";
	
	$sql = "update page set is_active = if(is_active = '1','0', '1' ) where id='".$_POST['id']."'";	
	$res = mysqli_query($link, $sql) or die(mysqli_error($link));
	
	if($res)
	{
		$sql = "select is_active from page where id='".$_POST['id']."'";
		$res_inner = mysqli_query($link, $sql) or die(mysqli_error($link));
		$row_inner = mysqli_fetch_array($res_inner);
		
		$returnArray['success'] = true;
		$returnArray['msg'] = " Page status has been successfully changed.";
...
elseif($_POST['func'] == "add")
{
	...
	{
		$returnArray['success'] = true;
		$returnArray['msg'] = "Page has been added successfully";
...
{
	$returnArray['success'] = false;
	$returnArray['msg'] = "Page deletion is not successful.";
	$sql = "select delete_p 
			from role_module_permission 
			join admin on (admin.admin_role = role_module_permission.role_id)
			where admin_id='".$_POST['admin_id']."' and module_id='19'"; 
	$res = mysqli_query($link, $sql) or die(mysqli_error($link));
	...
	if($row['delete_p'])
	{
		...
		{
			$returnArray['success'] = true;
			$returnArray['msg'] = "Page deletion is successful.";
		}
	}
	else
	{
...
elseif($_POST['func'] == 'edit_save')
{
	...
	{
		$returnArray['success'] = true;
		$returnArray['msg'] = "Page has been updated successfully";
