...
if($funcName == 'lab_registration')
{
	$returnArray= array();
	$sql = "select LabId from lab where email='".$_REQUEST['email']."'";
	$res = mysqli_query($link, $sql) or die(mysqli_error($link));
	$num_lab = mysqli_num_rows($res);
	if($num_lab > 0)
	{
		$returnArray['success'] = false;
		$returnArray['msg'] = "Lab with given eMaild id is already registered";
	}
	else
	{
		$sql = "insert lab set
		...
		if($res)
		{
			$insert_lab_id = mysqli_insert_id($link);
			$digits = 4;
			...
			$to = $_REQUEST['email'];
			$subject = "Welcome Lab Management - ".$_REQUEST['name'].", Remote Health is willing to verify your identity!";
...
<table border="0" cellspacing="0" cellpadding="0" width="100%">
  ...
  <tr>
    <td valign="top" >
      <b>Dear Supervisor, Lab - '.$_REQUEST['name'].'</b><br/>      
    ...
    <td valign="top" align="center">
     ...
     <br/>
      ...
      concerns with your Doctor. 
			...
			$returnArray['insert_lab_id'] = $insert_lab_id;
...
// Lab Sign In
elseif($funcName == 'lab_login')
{
	$returnArray= array();
	$sql = "select * from lab where email='".trim($_REQUEST['lab_login_email'])."'";
	$res = mysqli_query($link, $sql) or die(mysqli_error($link));
	...
	if($num == 1)
	{
		if($row['is_active'] == 1)
		{
			if(verifyPasswordHash($_REQUEST['lab_login_password'],$row['password']))
			{
				$returnArray['success'] = true;
...
// List of Labs
elseif($funcName == 'getLabList')
{
	$returnArray= array();
	$sql = "SELECT LabId, Name, Address, PinCode FROM lab";
	$res = mysqli_query($link, $sql) or die(mysqli_error($link));
	$num = mysqli_num_rows($res);
	if($num > 0)
	{
		...
		while($row = mysqli_fetch_array($res))
		{
			...
			$returnArray['data'][$i]['LabId'] = $row['LabId'];
