...
// start - nurse slot
elseif($funcName == "get_slot_by_nurse")
{
	$returnArray = array();
	$duration = 30;
...
elseif($funcName == 'get_user_appointments')
{
	...
	$sql = "SELECT 
			...
			left join specialization on (specialization.id = doctor.specialization_id) 
	...
	while($row = mysqli_fetch_array($res))
	{
		...
		if($row['file'])
		{
			$returnArray['previous']['data'][$i]['pres'] = base64_encode($row['file']);
		}
		else
...
elseif($funcName == 'get_doctor_appointments')
{
	...
	while($row = mysqli_fetch_array($res))
	{
		...
		if($row['dob'] = "0000-00-00 00:00:00")
			$row['age'] = date('Y') - date('Y',strtotime($row['dob'])). "Yr";
		else
			$row['age'] = "";
			
		if($row['sex'] == 'm') $row['gender'] = "Male";
		elseif($row['sex'] == 'f') $row['gender'] = "Female";
		elseif($row['sex'] == 'o') $row['gender'] = "Others";
		else $row['gender'] = "";
...
elseif($funcName == 'temp_subscribe')
{
	$returnArray = array();
	$apiKey = "46491382";
	$apiSecret = "4d6825e50fe5386203f4ec03ce0a495ed45cb843";
...
//		'role'       => Role::PUBLISHER,
//		'expireTime' => time()+(15 * 24 * 60 * 60), // in one week
//		'data'       => '',
//		'initialLayoutClassList' => array('focus')
//	));
	$token="test-token";
	$mnth = $_REQUEST['app_month']+1;
...
<table border="0" cellspacing="0" cellpadding="0" width="100%">
  ...
  <tr>
    <td valign="top" >
      ...
      The <a href="https://www.remotehealth.org">Remote Health</a> appointment with Dr. '.$row3['f_name'].' '.$row3['l_name'].' is scheduled on '.$_REQUEST['app_date'].'/ '.($_REQUEST['app_month']+1).'/'.$_REQUEST['app_year'].' at '.$_REQUEST['app_time'].'. you can also reschedule this appointment at least 30 minutes prior to this appointment.<br/>
...
If you have any questions regarding your appointment, please contact us. We will be happy to help you.  
  ...
  <tr>
    <td valign="top" align="center">
      ...
      Spend less time travelling and no waiting times in the traffic and in the office.
    </td>
  </tr>
    ...
    <td valign="top" align="center">
     ...
     <br/>
      ...
      concerns with your Doctor. 
		...
		$headers[] = 'From: Remote Health Appointment<no-reply@remotehealth.org>';
		$headers[] = 'Cc: sksafdar@Remote Health.in';
...
<table border="0" cellspacing="0" cellpadding="0" width="100%">
  ...
  <tr>
    <td valign="top" >
      ...
      New <a href="https://www.remotehealth.org">Remote Health</a> appointment is booked. Here are the details...<br/> <br/> 
...
/*start of opentok details*/
elseif($funcName == 'OpentokDets')
{
	$returnArray = array();
	$apiKey = "46491382";
	$apiSecret = "4d6825e50fe5386203f4ec03ce0a495ed45cb843";
	...
	$sql ="SELECT f_name,ddid as id FROM doctor WHERE id='".$docid."' UNION SELECT f_name, pdid as id FROM users WHERE id='".$userid."'";
	$res = mysqli_query($link, $sql) or die(mysqli_error($link).$sql);	
	//$row = mysqli_fetch_array($res);
	$i=0;
	$returnArray['apiKey'] = $apiKey;
	$returnArray['apiSecret'] = $apiSecret;		
	...
	{*/
		while($row = mysqli_fetch_array($res))
		{
			$returnArray['data'][$i]['name'] = stripslashes($row['f_name']);
			$returnArray['data'][$i]['id'] = $row['id'];
			$i++;
		}
		$returnArray['data'][0]['type'] = 'Doctor';
		$returnArray['data'][1]['type'] = 'Patient';
		echo json_encode($returnArray);
	//}
	/*if($row)
	{
		$returnArray['success'] = true;
		$returnArray['msg'] = "OpenTok details found.";
		$returnArray['apiKey'] = $apiKey;		
	...
	{
		$returnArray['success'] = false;
		$returnArray['msg'] = "OpenTok details not found. Try again.";
	}*/
}

/*end of opentok details*

/* Start of chat appointments */
elseif($funcName == 'doc_chat_appointments')
{
	$returnArray = array();
	$apiKey = "46491382";
	$apiSecret = "4d6825e50fe5386203f4ec03ce0a495ed45cb843";
	...
	$nssql = "";
	if($_REQUEST['app_nurse_schedule']) $nssql = "nurse_schedule_time='".$_REQUEST['app_nurse_schedule']."', ";
	$sql = "insert doctor_chat_appointments set
				user_id='".$_REQUEST['user_id']."',
				doc_id='".$_REQUEST['app_doc_id']."',
				year='".$_REQUEST['app_year']."',
	...
	if($res)
	{
		$returnArray['appt_id'] = mysqli_insert_id($link);
		$returnArray['success'] = true;
		$returnArray['msg'] = "Chat appointment booking is successful.";
		//=====================Booking confirmation mail to user ===========================//
		$sql = "select value_text from site_info where id='16'";
		$res2 = mysqli_query($link, $sql) or die(mysqli_error($link));
		$row = mysqli_fetch_array($res2);
		...
		$to = $row4['email_id'];
		$subject = "Welcome ".$row4['f_name'].", your chat appointment booking is Successful!";
		$content = '<html>
<head>
  <title></title>
</head>
...
If you have any questions regarding your appointment, please contact us. We will be happy to help you.  
  ...
  <tr>
	...
	{
		$returnArray['success'] = false;
		$returnArray['msg'] = "Chat appointment booking is not successful";
	}
	echo json_encode($returnArray);
}

/* End of chat appointments */
...
<table border="0" cellspacing="0" cellpadding="0" width="100%">
  ...
  <tr>
    <td valign="top" >
      <b>Dear '.$row4['f_name'].' '.$row4['l_name'].'</b><br/>  
      The <a href="https://www.remotehealth.org">Remote Health</a> appointment with Dr. '.$row3['f_name'].' '.$row3['l_name'].' is rescheduled on '.$_REQUEST['app_date'].'/ '.($_REQUEST['app_month']+1).'/'.$_REQUEST['app_year'].' at '.$_REQUEST['app_time'].'. you can also reschedule this appointment at least 30 minutes prior to this appointment.<br/>
...
<table border="0" cellspacing="0" cellpadding="0" width="100%">
  ...
  <tr>
    <td valign="top" >
      <b>Dear Dr. '.$row3['f_name'].' '.$row3['l_name'].'</b><br/>  
      An <a href="https://www.remotehealth.org">Remote Health</a> appointment is rescheduled. Here are the details...<br/> <br/> 
      	<b>Patient Name: <b>'.$row4['f_name'].' '.$row4['l_name'].'<br/>      	
      	 <b>Date: </b> '.$_REQUEST['app_date'].'/ '.($_REQUEST['app_month']+1).'/'.$_REQUEST['app_year'].' <br/>
      	 <b>Time: </b>'.$_REQUEST['app_time'].'<br/><br/>
 If you have any question, please feel free to contact us.
    ...
    <td valign="top" align="center">
     ...
     <br/>
      ...
      concerns with your Doctor. 
		...
		$headers[] = 'From: Remote Health Appointment<no-reply@remotehealth.org>';
		//$headers[] = 'Cc: birthdayarchive@example.com';
		$headers[] = 'Bcc: sksafdar@Remote Health.in';
...
elseif($funcName == "get_user_details_by_app_id")
{
	...
	if($row['sex'] == 'm') $row['gender'] = "Male";
	elseif($row['sex'] == 'm') $row['gender'] = "Female";
	elseif($row['sex'] == 'o') $row['gender'] = "Others";
	else $row['gender'] = "";
...
elseif($funcName == 'save_prescription')
{
	...
	{	
		...
		$image = time().".".$ext;
		if (move_uploaded_file($_FILES['file']['tmp_name'],"../prescription_file/".$image))
		{	
	...
	if($res)
	{
		...
		$file_link = base64_encode($file_link);
		//$returnArray['Retimage'] = $_REQUEST['orgFile'];
...
elseif($funcName == 'save_prescription')
{
	...
	$ImagePath = "../prescription_file/".$ImageName;
	
	$sql = "delete from prescription where app_id = '".$_REQUEST['app_id']."'";
	$res = mysqli_query($link, $sql) or die(mysqli_error($link).$sql);	

	$sql = "insert prescription set app_id ='".$_REQUEST['app_id'] ."',file='". $_FILES['file']['name'] ."', create_date=NOW()";
	$res = mysqli_query($link, $sql) or die(mysqli_error($link).$sql);	
	if($res)
	{
		$id = mysqli_insert_id($link);
		//file_put_contents($ImagePath,base64_decode($ImageData));
		move_uploaded_file($_FILES["file"]["tmp_name"],"../prescription_file/".$_FILES['file']['name']);
		
		$returnArray['file_link'] = base64_encode($_FILES['file']['name']);
		$returnArray['success'] = true;
		$returnArray['msg'] = "Prescription saved successfully.";		
	}
	else
	{
		$returnArray['success'] = false;
		$returnArray['msg'] = "Upload failed. Please try again later or call technical team";
	}	
	echo json_encode($returnArray);
}


elseif($funcName == 'save_prescription_browse')
{
	//print_r($_REQUEST);
	//print_r($_FILES);
	$returnArray= array();
	...
	$sql = "insert prescription set app_id ='".$_REQUEST['app_id'] ."',file='pres_". $_REQUEST['app_id'].".".$ext ."', create_date=NOW()";
	$res = mysqli_query($link, $sql) or die(mysqli_error($link).$sql);	
	if($res)
	{
		$id = mysqli_insert_id($link);
		//file_put_contents($ImagePath,base64_decode($ImageData));
		move_uploaded_file($_FILES["file"]["tmp_name"],"../prescription_file/pres_".$_REQUEST['app_id'].".".$ext);
		
		$returnArray['file_link'] = base64_encode("pres_". $_REQUEST['app_id'] . "." .$ext);
		$returnArray['success'] = true;
		$returnArray['msg'] = "Prescription saved successfully.";		
	}
	else
...
<table border="0" cellspacing="0" cellpadding="0" width="100%">
  ...
  <tr>
    <td valign="top" >
      <b>Dear '.$row4['f_name'].' '.$row4['l_name'].'</b><br/>  
      The <a href="https://www.remotehealth.org">Remote Health</a> appointment with Dr. '.$row3['f_name'].' '.$row3['l_name'].' on '.$row['day'].'/ '.($row['month']+1).'/'.$row['year'].' at '.$row['time'].' is cancelled. <br/>
...
<table border="0" cellspacing="0" cellpadding="0" width="100%">
  ...
  <tr>
    <td valign="top" >
      <b>Dear Dr. '.$row3['f_name'].' '.$row3['l_name'].'</b><br/>  
      An <a href="https://www.remotehealth.org">Remote Health</a> appointment is cancelled. Here are the details...<br/> <br/> 
      	<b>Patient Name: <b>'.$row4['f_name'].' '.$row4['l_name'].'<br/>      	
