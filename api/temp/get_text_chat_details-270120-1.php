...
if($funcName == 'check_subscription_validity')
{
	...
	$sql ="select * from message_subscription where user_id ='".$_POST['user_id']."' order by create_date desc limit 0, 1";
...
elseif($funcName == "save_message")
{
	...
	$sql = "select image from users where id='".$_POST['user_id']."'";
	$res = mysqli_query($link, $sql) or die(mysqli_error($link));
	$row = mysqli_fetch_array($res);
	$uimage = $row['image'];
	if(!$uimage) $uimage = "avatar.png";

	$sql = "select image from doctor where id='".$_POST['doc_id']."'";
	$res = mysqli_query($link, $sql) or die(mysqli_error($link));
	$row = mysqli_fetch_array($res);
	$dimage = $row['image'];
	if(!$dimage) $dimage = "no-doctor.png";

	$file_name = "../chat_messages/chat_".$_POST['user_id']."_".$_POST['doc_id'].".json";
	...
	if(file_exists($file_name))
	{
		...
		$chat_msg = array();
		$chat_msg['type'] = $_POST['type'];
		$chat_msg['msg'] = addslashes(trim($_POST['msg']));
		$chat_msg['time'] = time();
		$chat_msg['read'] = 0;
