...
// chat_".$_POST['user_id']."_".$_POST['doc_id'].".json
...
{
	$returnArray= array();
	$start_time    = strtotime ($_POST['start_time']); //change to strtotime
	$end_time      = strtotime ($_POST['end_time']); //change to strtotime
...
elseif($funcName == "get_slot_by_doc_id")
{
	...
	$sql = "select time 
			...
			where			
			doc_id='".$_POST['doc_id']."' and
			year='".$_POST['year']."' and
			month='".$_POST['month']."' and
			day='".$_POST['date']."' and status='1'";
	...
	$sql ="select * from doctor_video_timing where doc_id = '".$_REQUEST['doc_id']."' and  day_id='".$_POST['day_id']."'";
...
elseif($funcName == "get_slot_by_nurse_id")
{
	...
	$sql = "select time 
			...
			year='".$_POST['year']."' and
			month='".$_POST['month']."' and
			day='".$_POST['date']."' and status='1' and relase_nurse='1'";
...
elseif($funcName == 'get_user_appointments')
{
	...
	$sql = "SELECT 
			...
			WHERE user_id='".$_POST['user_id']."' and status='1' and relase_nurse='0' and end_call='0' and TIMESTAMP(ADDTIME(app_date_time,'00:15:00')) > TIMESTAMP('$today') and status = '1' order by app_date_time asc";
	...
	$sql = "SELECT 
			...
			join specialization on (specialization.id = doctor.specialization_id) 
			left join nurse_provided_services on(nurse_provided_services.id = doctor.service_id)
			WHERE (user_id='".$_POST['user_id']."' and status='1' and relase_nurse='1') 
				or
				(user_id='".$_POST['user_id']."' and status='1' and end_call='1')
				or
				( user_id='".$_POST['user_id']."' and status='1' and TIMESTAMP(ADDTIME(app_date_time,'00:15:00')) < TIMESTAMP('$today'))  order by app_date_time desc";
...
{
	$returnArray = array();
	$mnth = $_POST['app_month'] < 10 ? '0'.$_POST['app_month']+1 : $_POST['app_month']+1;
	$dt = $_POST['app_date'] < 10 ? '0'.$_POST['app_date'] : $_POST['app_date'];

	$app_date_time = $_POST['app_year']."-".$mnth."-".$dt." ".$_POST['app_time'].":00";

	$nssql = "";
	if($_POST['app_nurse_schedule']) $nssql = "nurse_schedule_time='".$_POST['app_nurse_schedule']."', ";

	$sql = "insert doctor_appointment set
				user_id='".$_POST['user_id']."',
				doc_id='".$_POST['app_doc_id']."',
				year='".$_POST['app_year']."',
				month='".$_POST['app_month']."',
				day='".$_POST['app_date']."',
				time ='".$_POST['app_time']."', $nssql
	...
	if($res)
	{
		...
		$sql = "select * from doctor where id='".$_POST['app_doc_id']."'";
...
<table border="0" cellspacing="0" cellpadding="0" width="100%">
  ...
  <tr>
    <td valign="top" >
      ...
      The <a href="https://www.actora.us">Actora</a> appointment with Dr. '.$row3['f_name'].' '.$row3['l_name'].' is scheduled on '.$_POST['app_date'].'/ '.($_POST['app_month']+1).'/'.$_POST['app_year'].' at '.$_POST['app_time'].'. you can also reschedule this appointment at least 30 minutes prior to this appointment.<br/>
    ...
    <td valign="top" >
      ...
      New <a href="https://www.actora.us">Actora</a> appointment is booked. Here are the details...<br/> <br/> 
      	<b>Patient Name: <b>'.$row4['f_name'].' '.$row4['l_name'].'<br/>      	
      	 <b>Date: </b> '.$_POST['app_date'].'/ '.($_POST['app_month']+1).'/'.$_POST['app_year'].' <br/>
      	 <b>Time: </b>'.$_POST['app_time'].'<br/><br/>
...
elseif($funcName == 'reschedule_appt')
{
	...
	if($res)
	{
		...
		$sql = "select user_id, doc_id from doctor_appointment where id='".$_POST['appt_id']."'";
		$res2 = mysqli_query($link, $sql) or die(mysqli_error($link));
...
<table border="0" cellspacing="0" cellpadding="0" width="100%">
  ...
  <tr>
    <td valign="top" >
      <b>Dear '.$row4['f_name'].' '.$row4['l_name'].'</b><br/>  
      The <a href="https://www.actora.us">Actora</a> appointment with Dr. '.$row3['f_name'].' '.$row3['l_name'].' is rescheduled on '.$_POST['app_date'].'/ '.($_POST['app_month']+1).'/'.$_POST['app_year'].' at '.$_POST['app_time'].'. you can also reschedule this appointment at least 30 minutes prior to this appointment.<br/>
...
elseif($funcName == "user_initiate_chat")
{
	...
	$sql ="insert initiate_chat set user_initiation='".$_POST['appt_id']."'";
	$res = mysqli_query($link, $sql) or die(mysqli_error($link).$sql);
	if($res)
	{
		$returnArray['success'] = true;
...
elseif($funcName == "check_user_initiate_chat")
{
	...
	$sql = "select id from initiate_chat where user_initiation='".$_POST['appt_id']."'";
	$res = mysqli_query($link, $sql) or die(mysqli_error($link).$sql);
	$num = mysqli_num_rows($res);
	if($num > 0)
	{
...
elseif($funcName == "get_appt_details")
{
	...
	WHERE doctor_appointment.id='". $_POST['appt_id']."' ";
...
elseif($funcName == "save_past_appt_note")
{
	...
	$sql = "update doctor_appointment set `note`= '".addslashes(trim($_POST['note_text']))."' where id='".$_POST['appt_id']."'";
	$res = mysqli_query($link, $sql) or die(mysqli_error($link).$sql);
	if($res)
	{
		$returnArray['success'] = true;
...
elseif($funcName == "show_past_appt_note")
{
	...
	$sql = "select `note` from doctor_appointment where id='".$_POST['appt_id']."'";
	$res = mysqli_query($link, $sql) or die(mysqli_error($link).$sql);
	$row = mysqli_fetch_array($res); 
	if($row['note'])
	{
...
elseif($funcName == "get_user_details_by_app_id")
{
	...
	$sql= "select *, prescription.id as pres_id from users 
			...
			left join prescription on(prescription. app_id = doctor_appointment.id)
			where doctor_appointment.id='".$_POST['app_id']."'";
	$res = mysqli_query($link, $sql) or die(mysqli_error($link).$sql);
	$row = mysqli_fetch_array($res); 		
	...
	{
		$file_link = $row['pres_id']."_".$_POST['app_id'].'_'.$row['file'];
...
elseif($funcName == 'save_prescription')
{
	...
	if($_FILES['file']['name'])
	{	
		$ext = pathinfo($_POST['orgFile'], PATHINFO_EXTENSION);
		$image = time().".".$ext;
	...
	$sql = "delete from prescription where app_id = '".$_POST['app_id']."'";
	$res = mysqli_query($link, $sql) or die(mysqli_error($link).$sql);	

	$sql = "insert prescription set app_id ='".$_POST['app_id'] ."',file='". $image ."', create_date=NOW()";
	$res = mysqli_query($link, $sql) or die(mysqli_error($link).$sql);	
	if($res)
	{
		$id = mysqli_insert_id($link);
		$file_link = $id."_".$_POST['app_id'].'_'.$image;
		$file_link = base64_encode($file_link);
...
elseif($funcName == "relase_nurse")
{
	...
	$sql = "update doctor_appointment set relase_nurse='1' where id='".$_POST['appt_id']."'";
	$res = mysqli_query($link, $sql) or die(mysqli_error($link).$sql);
	if($res)
	{
		$returnArray['success'] = true;
...
elseif($funcName == "end_call")
{
	$returnArray = array();
	$returnArray['success'] = false;
	$sql = "update doctor_appointment set end_call='1' where id='".$_POST['appt_id']."'";
	$res = mysqli_query($link, $sql) or die(mysqli_error($link).$sql);
	if($res)
	{
		$returnArray['success'] = true;
...
elseif($funcName == "appt_cancel")
{
	...
	$sql = "update doctor_appointment set status='0' where id='". $_POST['appt_id']."' ";
	$res = mysqli_query($link, $sql) or die(mysqli_error($link).$sql);
