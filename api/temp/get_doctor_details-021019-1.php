...
elseif($funcName == "get_specialist_doctor")
{
	...
	$sql ="select * from specialization where id='". $_POST['specialization_id'] ."'";
	$res = mysqli_query($link, $sql) or die(mysqli_error($link).$sql);
	$row = mysqli_fetch_array($res);
	$returnArray['per_visit_change'] = $row['per_visit_change'];

	$sql = "select * from doctor where is_active = '1' and specialization_id='".$_POST['specialization_id']."' and type in ('d','jd')";
	$res = mysqli_query($link, $sql) or die(mysqli_error($link));
	$returnArray['success'] = false;
...
elseif($funcName == "get_specialist_nurse")
{
	...
	$sql = "select * from doctor where is_active = '1' and specialization_id='".$_POST['specialization_id']."' and type in ('jn','sn')";
	$res = mysqli_query($link, $sql) or die(mysqli_error($link));
	$returnArray['success'] = false;
...
{//print_r($_REQUEST);
	$returnArray= array();
	$sql = "select * from doctor where id='".$_POST['doc_id']."'";
	...
	$sql = "select * from nurse_fees where nurse_id = '". $_POST['doc_id'] ."'";
	...
	$sql = "select service_id from nurse_service_selected where nurse_id='".$_POST['doc_id']."'";
	...
	$sql = "select * from review join users on (review.user_id= users.id) where doctor_id='".$_POST['doc_id']."' and review.is_active='1'";
	$res = mysqli_query($link, $sql) or die(mysqli_error($link));
	$i=0;
...
elseif($funcName == "doctor_registration")
{
	...
	else
	{
		...
		$sql = "insert doctor set
			...
			phy_id = '".addslashes(trim($_POST['physician_id']))."',
		...
		if($res)
		{
			...
			$to = $_POST['email'];
			$subject = "Welcome Dr. ".$_POST['f_name'].", Actora is willing to verify your identity!";
...
<table border="0" cellspacing="0" cellpadding="0" width="100%">
  ...
  <tr>
    <td valign="top" >
      <b>Dear Dr. '.$_POST['f_name'].' '.$_POST['l_name'].'</b><br/>      
...
{
	$returnArray= array();
	$sql = "update doctor set is_active='1' where id='".$_POST['doctor_id']."'";
	$res = mysqli_query($link, $sql) or die(mysqli_error($link));
	if($res)
	{
		$returnArray['success'] = true;
		...
		$sql = "select * from doctor where id='".$_POST['doctor_id']."'";
...
elseif($funcName == "get_service_nurse")
{
	...
	//$sql ="select * from nurse_provided_services where id='". $_POST['service_id'] ."'";
	//$res = mysqli_query($link, $sql) or die(mysqli_error($link).$sql);
	//$row = mysqli_fetch_array($res);
	$returnArray['per_visit_change'] = "TO BE WORKED ON";

	$searched_nurse_id = array();
	$sql = "select nurse_id from nurse_service_selected where service_id = '".$_POST['service_id']."'";
	$res = mysqli_query($link, $sql) or die(mysqli_error($link));
	while($row = mysqli_fetch_array($res))
	{
...
elseif($funcName == "get_service_nurse_day_avail")
{
	...
	$sql = "select doc_id from doctor_video_timing where day_id='". $_POST['day_id']."'";
	$res = mysqli_query($link, $sql) or die(mysqli_error($link));
	while($row = mysqli_fetch_array($res))
	{
...
elseif($funcName == "change_password")
{
	...
	{
		$sql = "select * from doctor where id='".trim($_POST['user_id'])."'";
		//echo $sql;
		$res = mysqli_query($link, $sql) or die(mysqli_error($link));
		$row = mysqli_fetch_array($res);

		$check = verifyPasswordHash($_POST['old_password'],$row['password']);
...
elseif($funcName == "doctor_reset_password")
{
	...
	$sql = "select * from doctor where email_id='".$_POST['doctor_reset_email']."'";
	$res = mysqli_query($link, $sql) or die(mysqli_error($link));
	$row = mysqli_fetch_array($res);
	$num = mysqli_num_rows($res);
	if($num > 0)
    ...
    <td valign="top" align="center">
     ...
     <br/>
      ...
      concerns with your Doctor. 
			...
			// Additional headers
			$headers[] = 'To: '.$row['f_name'].' '.$row['l_name'] .' <'.$_POST['doctor_reset_email'].'>';
...
elseif($funcName == "doctor_update_profile")
{
	...
	$sql = "update doctor set
		...
		phy_id = '".addslashes(trim($_POST['physician_id']))."',
		about = '".addslashes(trim($_POST['about']))."',
		type = '".addslashes(trim($_POST['nurse_type']))."',		
		designation = '".addslashes(trim($_POST['designation']))."',
		specialization_id = '".$_POST['specialization_id']."',
		$certificateSql
		language_known = '".json_encode($_POST['lang'])."' where id='".$_POST['doctor_id']."'";

		//service_id = '".addslashes(trim($_POST['service_id']))."',
	$res = mysqli_query($link, $sql) or die(mysqli_error($link).$sql);			

	if($res)
	{
		if(isset($_POST['nurse_type']))
		{
			$sql = "delete from nurse_fees where nurse_id='".$_POST['doctor_id']."'";
		...
		{
			$sql = "delete from nurse_service_selected where nurse_id='".$_POST['doctor_id']."'";
			mysqli_query($link, $sql) or die(mysqli_error($link));
...
elseif($funcName == "user_change_image")
{
	...
	$sql = "update doctor set image ='".$image ."' where id='".$_POST['user_id']."'";
	$res = mysqli_query($link, $sql) or die(mysqli_error($link).$sql);	
	if($res)
	{
		$returnArray['success'] = true;
...
{//print_r($_REQUEST);
	...
	$sql = "select doctor.* from doctor
			join doctor_appointment on (doctor_appointment.doc_id = doctor.id) where doctor_appointment.id='".$_POST['appt_id']."'";
...
elseif($funcName == "jr_doc_registration")
{
	...
	else
	{
		...
		if($res)
		{
			...
			$to = $_POST['email'];
			$subject = "Welcome Dr. ".$_POST['f_name'].", Actora is willing to add you in Actora Family!";
			$content = '<html>
<head>
  <title></title>
</head>
...
elseif($funcName == "nurse_registration")
{
	...
	else
	{
		...
		if($res)
		{
			...
			$to = $_POST['email'];
			$subject = "Welcome ".$_POST['f_name'].", Actora is willing to add you in Actora Family!";
			$content = '<html>
<head>
  <title></title>
</head>
