...
elseif($funcName == 'pharmacy_login')
{
	...
	if($num == 1)
	{
		if($row['is_active'] == 1)
		{
			...
			{
				$returnArray['success'] = true;
				$returnArray['PharmId'] = $row["PharmId"];
...
elseif($funcName == 'pharmacy_trans')
{
	...
	{
		$returnArray['success'] = true;
		$returnArray['prescId'] = $row["prescId"];
		$returnArray['FileName'] = stripslashes($row["file"]);
		$returnArray['UploadedTime'] = stripslashes($row["UpTime"]);
		$returnArray['UserId'] = stripslashes($row["UserId"]);
		$returnArray['DocId'] = stripslashes($row["DocId"]);
		$returnArray['userName'] = stripslashes($row["userName"]);
		$returnArray['docName'] = stripslashes($row["docName"]);
	}*/
	if($num > 0)
	{
		$returnArray['success'] = true;
