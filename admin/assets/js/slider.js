checkPermission(window.sessionStorage.getItem("admin_id"), 23, 'view'); // admin_id, module_id, role_type
jQuery(document).ready(function() {	
$(".username").html(window.sessionStorage.getItem("admin_name"));
	$.post( api_url + "/slider.php", { func:"list"})
		.done(function(data){
			//alert(data);
			...
			html +=		'			<th>#</th>';
			html +=		'			<th>Image </th>';
			html +=		'			<th style="width:30%">Text </th>';
			html +=		'			<th style="width:20%">link</th>';
			html +=		'			<th>Status</th>';
            html +=		'           <th>&nbsp;</th>';
			html +=		'		</tr>';
			html +=		'	</thead>';
			...
			{				
				html +=		'	<tr>';
				html +=		'		<td>'+ i +'</td>';
				html +=		'		<td><img width="100px" src="'+site_url+"images/slider_images/"+ data.data[key]['image'] +'"></td>';
				html +=		'		<td>'+ data.data[key]['text1']+"<br/>"+data.data[key]['text2']+"<br/>"+data.data[key]['text3']+"<br/>"+data.data[key]['text4'] +'</td>';
				html +=		'<td><a href="'+ data.data[key]['link1_l']+'">'+data.data[key]['link1']+'</a><br/><a href="'+data.data[key]['link2_l']+'">'+data.data[key]['link2'] +'</a></td>';	
			...
			$(".portlet-body").html(html).find(".change_status a").click(function(){
				...
				$.post( api_url + "/slider.php", { func:"status_change", id:id[1]})
					.done(function(data){
						//alert(data);
						data = JSON.parse(data);
						if(data.success)
			...
			$(".page-sidebar").load("nav.html",function(response, status){
				...
				$("#slider").addClass("active").addClass("open");
				if($("#slider span").hasClass("arrow"))
				{
					$("#slider span").addClass("open");	
				}
				$("#list_slider").addClass("active");
...
jQuery(document).on('click', '.edit_delete .edit_btn', function(e){
	var id = $(this).parent().attr('id').split("_");	
	window.location.href="add_slider.html?id="+id[1];
});
...
jQuery(document).on('click', '.edit_delete .dlt_btn', function(e){
	...
	if (r == true) {
    		$.post( api_url + "/slider.php", { func:"delete", id:id[1], admin_id: window.sessionStorage.getItem("admin_id")})
			.done(function(data){
				data = JSON.parse(data);
				if(data.success)
				{
