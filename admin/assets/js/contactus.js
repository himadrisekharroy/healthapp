checkPermission(window.sessionStorage.getItem("admin_id"), 21, 'view'); // admin_id, module_id, role_type
jQuery(document).ready(function() {	
$(".username").html(window.sessionStorage.getItem("admin_name"));
	$.post( api_url + "/contact_us.php", { func:"list"})
		.done(function(data){
			//alert(data);
			...
			{				
				...
				html +=		'		<td>'+ data.data[key]['ph'] +'</td>';
				html +=		'		<td vallign="top">'+ data.data[key]['msg'] +'</td>';
			...
			$(".page-sidebar").load("nav.html",function(response, status){
				...
				$("#contactus").addClass("active").addClass("open");
				if($("#contactus span").hasClass("arrow"))
				{
					$("#contactus span").addClass("open");	
				}
				$("#list_contactus").addClass("active");
