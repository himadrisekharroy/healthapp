checkPermission(window.sessionStorage.getItem("admin_id"), 3, 'view'); // admin_id, module_id, role_type
jQuery(document).ready(function() {	
$(".username").html(window.sessionStorage.getItem("admin_name"));
	$.post( api_url + "/sd_state.php", { func:"list"})
		.done(function(data){
			//alert(data);
			...
			{				
				...
				html +=		'		<td class="change_status" id="cs_' + data.data[key]['state_id'] + '"><a href="javascript:void(0);" class="btn mini green" title="Change Status"><i class="icon-unlock"></i> Active</a></td>';
				else
				html +=		'		<td class="change_status" id="cs_' + data.data[key]['state_id'] + '"><a href="javascript:void(0);" class="btn mini blue" title="Change Status"><i class="icon-lock"></i> Inactive</a></td>';
				
				html +=     '       <td class="edit_delete" id="ed_' + data.data[key]['state_id'] + '">';
			...
			$(".page-sidebar").load("nav.html",function(response, status){
				...
				$("#state").addClass("active").addClass("open");
				if($("#state span").hasClass("arrow"))
				{
					$("#state span").addClass("open");	
				}
				$("#list_state").addClass("active");
