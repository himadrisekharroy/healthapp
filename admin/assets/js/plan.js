checkPermission(window.sessionStorage.getItem("admin_id"), 14, 'view'); // admin_id, module_id, role_type
jQuery(document).ready(function() {	
$(".username").html(window.sessionStorage.getItem("admin_name"));
	$.post( api_url + "/sd_plan.php", { func:"list"})
		.done(function(data){
			//alert(data);
			...
			{				
				...
				html +=		'		<td class="change_status" id="cs_' + data.data[key]['plan_id'] + '"><a href="javascript:void(0);" class="btn mini green" title="Change Status"><i class="icon-unlock"></i> Active</a></td>';
				else
				html +=		'		<td class="change_status" id="cs_' + data.data[key]['plan_id'] + '"><a href="javascript:void(0);" class="btn mini blue" title="Change Status"><i class="icon-lock"></i> Inactive</a></td>';
				
				html +=     '       <td class="edit_delete" id="ed_' + data.data[key]['plan_id'] + '">';
			...
			$(".portlet-body").html(html).find(".change_status a").click(function(){
				...
				$.post( api_url + "/sd_plan.php", { func:"status_change", id:id[1]})
					.done(function(data){
						//alert(data);
						data = JSON.parse(data);
						if(data.success)
			...
			$(".page-sidebar").load("nav.html",function(response, status){
				...
				$("#plan").addClass("active").addClass("open");
				if($("#plan span").hasClass("arrow"))
				{
					$("#plan span").addClass("open");	
				}
				$("#list_plan").addClass("active");
...
jQuery(document).on('click', '.edit_delete .edit_btn', function(e){
	var id = $(this).parent().attr('id').split("_");	
	window.location.href="add_plan.html?id="+id[1];
});
...
jQuery(document).on('click', '.edit_delete .dlt_btn', function(e){
	...
	if (r == true) {
    		$.post( api_url + "/sd_plan.php", { func:"delete", id:id[1], admin_id: window.sessionStorage.getItem("admin_id")})
			.done(function(data){
				data = JSON.parse(data);
				if(data.success)
				{
