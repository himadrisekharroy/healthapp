checkPermission(window.sessionStorage.getItem("admin_id"), 19, 'view'); // admin_id, module_id, role_type
jQuery(document).ready(function() {	
$(".username").html(window.sessionStorage.getItem("admin_name"));
	$.post( api_url + "/page.php", { func:"list"})
		.done(function(data){
			//alert(data);
			...
			$(".portlet-body").html(html).find(".change_status a").click(function(){
				...
				$.post( api_url + "/page.php", { func:"status_change", id:id[1]})
					.done(function(data){
						//alert(data);
						data = JSON.parse(data);
						if(data.success)
			...
			$(".page-sidebar").load("nav.html",function(response, status){
				...
				$("#page").addClass("active").addClass("open");
				if($("#page span").hasClass("arrow"))
				{
					$("#page span").addClass("open");	
				}
...
jQuery(document).on('click', '.edit_delete .edit_btn', function(e){
	var id = $(this).parent().attr('id').split("_");	
	window.location.href="add_page.html?id="+id[1];
});
...
jQuery(document).on('click', '.edit_delete .dlt_btn', function(e){
	...
	if (r == true) {
    		$.post( api_url + "/page.php", { func:"delete", id:id[1], admin_id: window.sessionStorage.getItem("admin_id")})
			.done(function(data){
				console.log(data);
				data = JSON.parse(data);
				if(data.success)
